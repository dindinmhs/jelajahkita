...
export default function AIChatbot({ map, umkmMarkers }: AIChatbotProps) {
  ...
  const [isAudioEnabled, setIsAudioEnabled] = useState(true); // ✅ Add audio toggle
  const [connectionStatus, setConnectionStatus] = useState<
    "disconnected" | "connecting" | "connected"
  >("disconnected");
  const [error, setError] = useState<string | null>(null);
  const [aiResponse, setAiResponse] = useState<string>("");
  const [isPlayingAudio, setIsPlayingAudio] = useState(false); // ✅ Add audio playing state
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setAiResponse("");
    resetAudioState(); // ✅ Reset audio before starting
    ...
    try {
      switch (functionName) {
        case 'show_umkm_details': {
          ...
          // ✅ FIX: Set selected UMKM dengan format yang benar
          setSelectedUmkm({
            id: umkm.umkm_id,
            name: umkm.name,
          ...
          // ✅ FIX: Set sidebar view dan buka sidebar secara eksplisit
        ...
        case 'navigate_to_umkm': {
          ...
          // ✅ FIX: startNavigation menerima 2 parameter terpisah (name, coordinates)
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="bg-gradient-to-r from-purple-500 to-blue-600 hover:from-purple-600 hover:to-blue-700 shadow-xl rounded-full w-12 h-12 flex items-center justify-center transition-all animate-pulse"
        title="AI Assistant"
      >
  ...
  return (
    <div className="absolute bottom-20 right-6 z-50 w-96 bg-white rounded-2xl shadow-2xl border border-gray-200">
      {/* Header */}
      ...
      {/* Response */}
      {aiResponse && (
        <div className="p-4 bg-blue-50 border-b max-h-40 overflow-y-auto">
...
• Navigasi ke [nama UMKM]"
          className="w-full p-3 text-sm border rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-purple-500 mb-3"
          rows={3}
          disabled={isLoading}
          onKeyDown={(e) => {
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            className="px-4 py-2 bg-gradient-to-r from-purple-500 to-blue-600 text-white text-sm font-medium rounded-lg hover:from-purple-600 hover:to-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all flex items-center gap-2"
          >
            {isLoading ? (
              <>
